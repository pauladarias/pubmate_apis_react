...
function AddPub() {
  ...
  const postPub=async()=> {
    ...
    .then( 
      ...
      console.log(setAddPub({ ...addPub, ...newData}))
      //setAddPub(response.config.data)
      //setAddPub(data.concat(response.config.data))
    })
        ...
        <div id="Name"  className="form" style={{display: "block"}}>
          <form className="addpub__form">
            ...
            <div className="addpub__buttons addpub__buttons_name">
              <button className="addpub__save" onClick={(e) => postPub(e)}>SAVE</button>
